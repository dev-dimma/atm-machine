...
function WithdrawalAmount() {
  ...
  const withdrawalMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      navigate("/withdrawal/result", {
        state: { success: true, message: "Withdrawal successful!" },
      });
    },
    onError: (error) => {
      navigate("/withdrawal/result", {
  ...
  const handleWithdrawal = (e) => {
    ...
    withdrawalMutation.mutate(Number(amount));
  };
  ...
  return (
    <div className="bg-primary text-white p-8 rounded-lg shadow-lg w-full max-w-md">
      ...
      <form onSubmit={handleWithdrawal} className="space-y-4">
        ...
        >
          {withdrawalMutation.isPending ? "Processing..." : "Proceed"}
        </button>
        <button
          type="button"
